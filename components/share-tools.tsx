...
export function ShareTools({ profileUrl, displayName }: { profileUrl: string; displayName: string }) {
  ...
  useEffect(() => {
    ...
    import("qrcode")
      .then(({ default: QRCode }) =>
        QRCode.toDataURL(qrEntryUrl, { width: 900, margin: 2, errorCorrectionLevel: "H", color: { dark: "#10233a", light: "#ffffff" } })
      )
      .then(setQrUrl)
      .catch(() => undefined);
  }, [profileUrl]);
  ...
  const writeNfc = async () => {
    ...
    try {
      ...
      const nfcEntryUrl = createProfileEntryUrl(profileUrl, "nfc", window.location.origin);
      await writer.write(nfcEntryUrl);
  ...
  return (
    <div className="share-tools-grid">
      <section className="settings-panel share-tool-panel">
        <div className="settings-panel-heading"><span className="panel-icon"><QrCode size={20} /></span><div><h2>Saját QR-kód</h2><p>Beolvasáskor mindig a nyilvános névjegyoldal nyílik meg.</p></div></div>
        ...
        <p className="share-open-note">A QR-kód nem tartalmaz kontaktfájlt. A kapcsolat mentése csak a megnyíló névjegyoldal külön gombjával indul.</p>
      ...
      <section className="settings-panel share-tool-panel">
        <div className="settings-panel-heading"><span className="panel-icon"><Nfc size={20} /></span><div><h2>NFC-kártya beállítása</h2><p>Az NFC is közvetlenül a nyilvános névjegyoldalt nyitja meg.</p></div></div>
