...
export function MiniProfilePreview({ profile }: { profile: Profile }) {
  return (
    <div className={`mini-profile theme-${profile.theme}`} style={{ "--preview-accent": profile.accent_color } as React.CSSProperties}>
      ...
      {profile.avatar_url ? <SmartProfileImage src={profile.avatar_url} alt="" width={82} height={82} className="mini-avatar" /> : <div className="mini-avatar avatar-fallback">{initials(profile.display_name)}</div>}
